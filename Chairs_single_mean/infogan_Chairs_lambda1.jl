...
function initwq(atype, cdim=20, ndisc=3, gdim=1, winit=0.02)
    ...
    #FC output
    push!(w, winit*randn(cdim+1, 128))
    push!(w, zeros(cdim+1, 1))
    return convert_weights(w,atype), m
end
...
function qloss(wgq, mg, mq, z, c, g; atype=KnetArray{Float32}, fix_std=true)
    ...
    for i=1:ndisc
        ...
        disc_cross_ent = -mean(sum(log.(q_c_given_x_disc.+1e-8) .* cc, 1))
        disc_ent = -mean(sum(log.(0.05+1e-8) .* cc, 1))
        disc_loss += -disc_ent + disc_cross_ent  # negative of mutual info
    end
    
    #continuous code
    ncont = 1;
    mu = q_c_given_x_cont   # first row is for mean
    nll_gauss = 0
    for i=1:ncont
