...
function main(args)
    ...
    for epoch = 1:o[:epochs]
        ...
        @time for (x,y) in dtrn
            ...
            #train Q
            qloss_current = train_qnet!(wgq,mg,mq,z,c,optgq,o)
            push!(L1_array, -qloss_current)
            qlossval += qloss_current
        end
        ...
        if o[:outdir] != nothing #&& epoch % 5==0
            ...
            filepath = joinpath(o[:outdir],"L1_values",filename)
            save(filepath, "L1", L1_array)
...
function initwg(atype=Array{Float32}, zdim=62, cdim=10, winit=0.02)
    #input: z+c=62+10=72 dim
    w = Any[]
    m = Any[]

    # 2 dense layers combined with batch normalization layers
    push!(w, winit*randn(1024,zdim+cdim))
    push!(w, bnparams(1024))
    push!(m, bnmoments())
...
function gnet(wg,mg,z,c; training=true)  #add c,g
    x0 = vcat(z,c)
...
function initwq(atype, cdim=10, winit=0.02)
    w = Any[]
    m = Any[]
...
function qnet(wq,mq,x; training=true, alpha=0.1)
    ...
    #FC.output
    x5 = wq[end-1]*x4 .+ wq[end] #shape: (cdim,batchsize)
...
function qloss(wgq, mg, mq, z, c; atype=KnetArray{Float32}, fix_std=true)
    fake_images = gnet(wgq[:wg],mg,z,c; training=true)
    q_c_given_x_disc = qnet(wgq[:wq],mq,fake_images) #shape (cdim,batchsize and gdim,batchsize)
    
    disc_cross_ent = -mean(sum(log.(q_c_given_x_disc +1e-8) .* c, 1))
    disc_ent = -mean(sum(log.(0.1+1e-8) .* c, 1))
    
    disc_loss = -disc_ent + disc_cross_ent  # negative of mutual info.
    #L1 = disc_ent - disc_cross_ent
    return disc_loss 
end
